...
// src/index.ts
async function callClickUpApi(path, method, apiKey, body) {
  let apiVersion = "v2";
...
__name(callClickUpApi, "callClickUpApi");
  ...
  async init() {
    ...
    this.server.tool(
      ...
      async ({ listId, ...params }) => {
        ...
        Object.entries(params).forEach(([k, v]) => {
          ...
          else if (v !== void 0) query.append(k, String(v));
        });
    ...
    this.server.tool(
      ...
      async ({ taskId, comment_text, assignee, notify_all }) => {
        ...
        if (!apiKey)
          return { content: [{ type: "text", text: "API key missing." }] };
        const commentData = { comment_text };
        if (assignee) commentData.assignee = assignee;
        if (notify_all !== void 0) commentData.notify_all = notify_all;
        const result = await callClickUpApi(
          `task/${taskId}/comment`,
          "POST",
          apiKey,
...
// .wrangler/tmp/bundle-4jWaMM/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-4jWaMM/middleware-loader.entry.ts
