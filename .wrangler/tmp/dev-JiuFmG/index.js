...
// src/tools/authorization.ts
function registerAuthorizationTools(server, getApiKey, callClickUpApi) {
  ...
  server.resource(
    "clickup_user",
    new ResourceTemplate("clickup://user", { list: void 0 }),
    async (uri) => {
      const apiKey = getApiKey();
      if (!apiKey) {
        return {
...
__name(registerAuthorizationTools, "registerAuthorizationTools");

// src/tools/space.ts
function registerSpaceTools(server, getApiKey, callClickUpApi) {
  server.tool(
    "getSpaces",
    ...
    async ({ teamId, archived }) => {
      ...
      const query = archived !== void 0 ? `?archived=${archived}` : "";
      const result = await callClickUpApi(
  ...
  server.tool(
    ...
    async ({ workspaceId, name, multipleAssignees, features: features2 }) => {
      const apiKey = getApiKey();
      if (!apiKey)
        return { content: [{ type: "text", text: "API key missing." }] };
      const payload = {
        ...
        features: {
          due_dates: features2.dueDates,
          time_tracking: features2.timeTracking,
          tags: features2.tags,
          time_estimates: features2.timeEstimates,
          checklists: features2.checklists,
          custom_fields: features2.customFields,
          remap_dependencies: features2.remapDependencies,
          dependency_warning: features2.dependencyWarning,
          portfolios: features2.portfolios
        }
      };
      const result = await callClickUpApi(
  ...
  server.tool("deleteSpace", { spaceId: z.string() }, async ({ spaceId }) => {
    ...
    if (!apiKey)
      return {
        content: [
          { type: "text", text: JSON.stringify({ error: "API key missing." }) }
        ]
      };
  ...
  server.resource(
    "clickup_space",
    new ResourceTemplate("clickup://space/{space_id}", { list: void 0 }),
    async (uri, { space_id }) => {
      const apiKey = getApiKey();
      if (!apiKey)
        return {
...
__name(registerSpaceTools, "registerSpaceTools");

// src/tools/folder.ts
function registerFolderTools(server, getApiKey, callClickUpApi) {
  ...
  server.resource(
    "clickup_folder",
    new ResourceTemplate("clickup://folder/{folder_id}", { list: void 0 }),
    async (uri, { folder_id }) => {
      const apiKey = getApiKey();
      if (!apiKey) {
        return {
...
__name(registerFolderTools, "registerFolderTools");

// src/tools/list.ts
function registerListTools(server, getApiKey, callClickUpApi) {
  ...
  server.tool(
    ...
    async ({
      ...
      if (dueDate !== void 0) body.due_date = dueDate;
      if (dueDateTime !== void 0) body.due_date_time = dueDateTime;
      if (priority !== void 0) body.priority = priority;
      if (assignee !== void 0) body.assignee = assignee;
      if (status !== void 0) body.status = status;
      if (unsetStatus !== void 0) body.unset_status = unsetStatus;
      const result = await callClickUpApi(
        `list/${listId}`,
        "PUT",
        apiKey,
  ...
  server.resource(
    "clickup_list",
    new ResourceTemplate("clickup://list/{list_id}", { list: void 0 }),
    async (uri, { list_id }) => {
      const apiKey = getApiKey();
      if (!apiKey)
        return {
...
__name(registerListTools, "registerListTools");

// src/tools/task.ts
var convertEpochToSGT = /* @__PURE__ */ __name((epochMs) => {
  const offsetMs = epochMs + 8 * 60 * 60 * 1e3;
...
function registerTaskTools(server, getApiKey, callClickUpApi) {
  server.tool(
    "getTasks",
    "Retrieve tasks/subtasks for a ClickUp list. This tool only filters by date_closed (when tasks were marked closed) and ignores all other date filters.",
    {
      list_id: z.number(),
    ...
    async ({ list_id, date_closed_gt, date_closed_lt, ...params }) => {
      ...
      if (date_closed_gt == null && date_closed_lt == null) {
        queryParams.date_closed_gt = 0;
      } else {
        if (date_closed_gt != null) queryParams.date_closed_gt = date_closed_gt;
        if (date_closed_lt != null) queryParams.date_closed_lt = date_closed_lt;
      }
      const invalid = ["date_closed_gt", "date_closed_lt"].filter(
        (k) => queryParams[k] !== void 0 && String(queryParams[k]).length !== 13
      );
      if (invalid.length) {
        return {
          content: [
            {
              type: "text",
              text: `Please supply 13\u2011digit Unix\u2011ms timestamps for: ${invalid.join(
                ", "
              )}`
            }
          ]
      ...
      for (const [k, v] of Object.entries(queryParams)) {
        ...
        else if (v !== void 0) qs.append(k, String(v));
      }
      const url = `list/${list_id}/task?${qs.toString()}`;
      const result = await callClickUpApi(url, "GET", apiKey);
      let tasks = result.tasks || [];
      tasks = tasks.filter((t) => {
        const dc = t.date_closed;
        return dc !== void 0 && (queryParams.date_closed_gt == null || dc > queryParams.date_closed_gt) && (queryParams.date_closed_lt == null || dc < queryParams.date_closed_lt);
      });
      const converted = tasks.map((t) => ({
        ...t,
        date_closed_sgt: t.date_closed ? convertEpochToSGT(t.date_closed) : void 0
      }));
      return {
        content: [
          {
            type: "text",
            text: JSON.stringify({ ...result, tasks: converted })
          }
        ]
      };
    }
  ...
  server.tool("getTask", { taskId: z.string() }, async ({ taskId }) => {
    ...
    const convertedTask = { ...result };
    dateFields.forEach((field) => {
      if (result[field]) {
        convertedTask[`${field}_sgt`] = convertEpochToSGT(result[field]);
      }
    });
    return { content: [{ type: "text", text: JSON.stringify(convertedTask) }] };
  });
  server.tool(
    "updateTask",
    {
  ...
  server.resource(
    "clickup_task",
    new ResourceTemplate("clickup://task/{task_id}", { list: void 0 }),
    async (uri, { task_id }) => {
      const apiKey = getApiKey();
      if (!apiKey) {
        return {
  ...
  server.resource(
    "clickup_list_tasks",
    new ResourceTemplate("clickup://list/{list_id}/tasks", { list: void 0 }),
    async (uri, { list_id }) => {
      const apiKey = getApiKey();
      if (!apiKey) {
        return {
...
function registerDocTools(server, getApiKey, callClickUpApi) {
  server.tool(
    "searchDocs",
    {
    ...
    async ({
      ...
      const qs = new URLSearchParams();
      if (id) qs.set("id", id);
      if (creator !== void 0) qs.set("creator", String(creator));
  ...
  server.tool(
    ...
    async ({ workspaceId, title: title2, content: bodyContent, parentDoc }) => {
      const apiKey = getApiKey();
      if (!apiKey)
        return { content: [{ type: "text", text: "API key missing." }] };
      const docData = { title: title2 };
...
__name(registerDocTools, "registerDocTools");

// src/index.ts
async function invokeClickUpApi(path, method, apiKey, body) {
  let apiVersion = "v2";
  ...
  try {
    ...
    return { error: error3.message || "Unknown error occurred" };
  }
}
__name(invokeClickUpApi, "invokeClickUpApi");
var MyMCP = class extends McpAgent {
  ...
  async init() {
    const getApiKey = /* @__PURE__ */ __name(() => {
      const store = unstable_context.getStore();
...
// .wrangler/tmp/bundle-Oj2WgL/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Oj2WgL/middleware-loader.entry.ts
